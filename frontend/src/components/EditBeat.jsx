...
import { X, Save, MapPin, AlertCircle, CheckCircle } from "lucide-react";
    ...
    const fetchBeat = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/get-beat/${id}`
        );
        setBeatName(response.data.beatName);
      } catch (err) {
        console.error("Error fetching beat:", err);
        setError("Failed to fetch beat details");
      }
    };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/edit-beat/${id}`,
        { beatName: beatName.trim() },
        { headers: { "Content-Type": "application/json" } }
      );

      setMessage(response.data.message);
      setTimeout(() => navigate("/view-beats"), 1000);
    } catch (err) {
      console.error("Update Error:", err);
      if (err.response) {
        setError(err.response.data.error || "Failed to update beat");
      } else {
        ...
        <div className="text-center mb-8 animate-fade-in">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 mt-3 max-w-xs mx-auto leading-5">
              Update location beat information below
            </p>
          </div>
        </div>
